...
if __name__ == "__main__":
    ...
    local_dir = args.local_dir + "/actor"
    ...
    model_name = hf_path.split("/")[-4]
    step_name = hf_path.split("/")[-3].split("_")[-1]
    # move all files from hf_path to save_path
    save_path = f"./checkpoints/{model_name}_step{step_name}"
    os.makedirs(save_path, exist_ok=True)
    print(f"Saving model to {save_path}")
    model.save_pretrained(save_path, state_dict=state_dict)
    for file in os.listdir(hf_path):
        shutil.copy(os.path.join(hf_path, file), os.path.join(save_path, file))
    del state_dict
    del model
