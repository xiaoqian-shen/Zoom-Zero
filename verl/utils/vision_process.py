...
def _read_video_opencv(video_path):
...
def fetch_video(ele: dict, image_factor: int = IMAGE_FACTOR, return_video_sample_fps: bool = False, resize: bool = True) -> torch.Tensor | list[Image.Image]:
    if isinstance(ele["video"], str):
        video_reader_backend = get_video_reader_backend()
        try:
            if ele["video"].endswith(".npy"):
                video = torch.tensor(np.load(ele["video"]))
                sample_fps = 1.0
            else:
