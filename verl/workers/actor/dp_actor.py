...
class DataParallelPPOActor(BasePPOActor):
    ...
    def _forward_micro_batch_gen(
        self, micro_batch: Dict[str, torch.Tensor]
    ) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        Returns:
            predicted_token_id: # (bs)
            log_probs: # (bs, vocab_size) for the next token
        """
        input_ids = micro_batch["input_ids"]
        attention_mask = micro_batch["attention_mask"]
        position_ids = micro_batch["position_ids"]
        ...
        with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
            if self.config.padding_free:
                ...
                logits_rmpad: torch.Tensor = output.logits.squeeze(0)  # (total_nnz or sp-chunk, vocab)
                ...
                # Get index of last non-pad token for each sequence in rmpad space
                lengths = attention_mask.sum(dim=1).to(torch.long)  # (bsz)
                last_indices = torch.cumsum(lengths, dim=0) - 1  # (bsz)
                last_indices = last_indices.to(logits_rmpad.device)

                final_logits = torch.index_select(logits_rmpad, dim=0, index=last_indices)  # (bsz, vocab)
                predicted_token_id = torch.argmax(final_logits, dim=-1)  # (bsz)
                log_probs = torch.log_softmax(final_logits, dim=-1)  # (bsz, vocab)
            else:
                output = self.actor_module(
                    input_ids=input_ids,
                    attention_mask=attention_mask,
                ...
                final_logits = logits[:, -1, :]  # (bsz, vocab_size)
                predicted_token_id = torch.argmax(final_logits, dim=-1)  # (bsz)
                log_probs = torch.log_softmax(final_logits, dim=-1)  # (bsz, vocab_size)

        return predicted_token_id, log_probs
    ...
    @torch.no_grad()
    def generate_sequences_zoom(self, data: DataProto) -> torch.Tensor:
        ...
        predicted_token_ids_lst = []
        for micro_batch in micro_batches:
            model_inputs = {**micro_batch.batch, **micro_batch.non_tensor_batch}
            predicted_token_id, log_probs = self._forward_micro_batch_gen(model_inputs)
            predicted_token_ids_lst.append(predicted_token_id)
        
        predicted_token_ids = torch.concat(predicted_token_ids_lst, dim=0)
    ...
    def update_policy(self, data: DataProto) -> Dict[str, Any]:
        ...
        for _ in range(self.config.ppo_epochs):
            for i, mini_batch in enumerate(mini_batches):
                ...
                print(f"gradient_accumulation: {gradient_accumulation}", "global_batch_size_per_device", self.config.global_batch_size_per_device)
