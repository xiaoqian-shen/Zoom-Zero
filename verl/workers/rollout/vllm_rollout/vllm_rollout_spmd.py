...
class vLLMRollout(BaseRollout):
    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        if "multi_modal_data" in non_tensor_batch:
            ...
            for raw_prompt_ids, multi_modal_data, mm_processor_kwargs in zip(
                non_tensor_batch.pop("raw_prompt_ids"), non_tensor_batch.pop("multi_modal_data"), non_tensor_batch.pop("mm_processor_kwargs")
            ):
    ...
    @torch.no_grad()
    def generate_sequences_zoom(self, prompts: DataProto, **kwargs) -> DataProto:
        input_ids: torch.Tensor = prompts.batch["input_ids"]  # (bs, prompt_length)
        attention_mask: torch.Tensor = prompts.batch["attention_mask"]
        position_ids: torch.Tensor = prompts.batch["position_ids"]
        eos_token_id: int = prompts.meta_info["eos_token_id"]
        ...
        # users can customize different sampling_params at different run
        # with self.update_sampling_params(**kwargs):
        sampling_kwargs = {"max_tokens": self.config.response_length, "detokenize": False}
        default_sampling_params = SamplingParams()
        for key in self.config.to_dict().keys():
            if hasattr(default_sampling_params, key):
                sampling_kwargs[key] = getattr(self.config, key)
        sampling_kwargs['n'] = 1
