...
def compute_grpo_sep_outcome_advantage(
    ...
    Returns:
        ...
        Returns: `(torch.Tensor)`
            shape: (bs, response_length)
    """
    exclude_reward = ["conf", "zoom"]
    all_scores = {}
    for key in token_level_rewards.keys():
        if key in exclude_reward:
            continue
        all_scores[key] = torch.zeros(len(eos_mask), dtype=torch.float32)
    for key, scores in token_level_rewards.items():
        if key in exclude_reward:
            continue
    ...
    scores = torch.stack(list(all_scores.values())).mean(dim=0)
    scores = scores.unsqueeze(-1).tile([1, response_length]) * eos_mask
    return scores, scores
...
def compute_grpo_select_outcome_advantage(
    ...
    Returns:
        ...
        Returns: `(torch.Tensor)`
            shape: (bs, response_length)
    """
    valid_reward = ["iou", "answer", "format", "zoom"]
    all_scores = {}
    for key in valid_reward:
        all_scores[key] = torch.zeros(len(eos_mask), dtype=torch.float32)
    for key, scores in token_level_rewards.items():
        if key not in valid_reward:
            continue
        response_length = eos_mask.shape[-1]
        id2score = defaultdict(list)
        id2mean, id2std = {}, {}
